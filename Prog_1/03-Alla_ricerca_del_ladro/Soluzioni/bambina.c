...
int iterazione(int* a){
    ...
    for(int i = 0; i < N && a[i] != 0; i++){ // Per tutti gli elementi all'interno dell'array
        if(i > 2){ // se ho due o più elementi precedenti controlla se l'elemento in posizione X è più grande della somma tra X-1 e X-2 
...
int ricorsione(int* a, int elem){
    if(a[elem] == 0){ // Se sono arrivato alla fine dell'array allora termina la ricorsione
        ...
        }else{ // se ho due o più elementi precedenti controlla se l'elemento in posizione X è più grande della somma tra X-1 e X-2
...
int main(void){
    printf("Calcolo utilizzando l'Iterazione: %d\n", iterazione(arr));
    printf("Calcolo utilizzando la Ricorsione: %d\n", ricorsione(arr, 0));
    return 0;
}